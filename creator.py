...
from PyQt5.QtWidgets import QVBoxLayout, QBoxLayout, QDesktopWidget, QInputDialog
...
class SmartNotes(QWidget):
    ...
    def new_note(self):
        title, okPressed = QInputDialog.getText(self, "Создание новой заметки", "Введите заголовок:")
        if okPressed and title:
            text, okPressed = QInputDialog.getText(self, "Создание новой заметки", "Введите текст заметки:")
            if okPressed and text:
                question, okPressed = QInputDialog.getItem(self, "Создание новой заметки", "Вам нужна картинка?",
                                                           ["Да", "Нет"])
                if okPressed and question == "Да":
                    pic = QFileDialog.getOpenFileName(self, 'Выбрать картинку', '')[0]
                    author = QInputDialog.getText(self, "Создание новой заметки", "Введите свое имя:",
                                                  getpass.getuser())
